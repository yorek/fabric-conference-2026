...
public class WebSocketRequestHandler
{
    ...
    private readonly Lights _lightsPlugin;
    private readonly ILogger<WebSocketRequestHandler> _logger;

    public WebSocketRequestHandler(Lights lightsPlugin, ILogger<WebSocketRequestHandler> logger)
    {
        ...
        try
        {
            switch (request.Action?.ToLowerInvariant())
            {
                ...
                case "togglelight":
                    ...
                    {
                        var isOn = request.State.ToLowerInvariant() == "on";
                        
                        var result = _lightsPlugin.ChangeLightState(request.Id.Value, isOn);
                        _logger.LogInformation($"Light state changed: {result?.Name} -> {(isOn ? "on" : "off")}");
